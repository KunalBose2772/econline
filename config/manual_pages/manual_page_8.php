<?php
// config/manual_pages/manual_page_8.php
return [
    'slug' => 'ec-villangam',
    'keyword' => 'ec villangam',
    'title' => 'EC Villangam Online Check: Tamil Nadu Property Encumbrance Guide',
    'meta_desc' => 'Perform an EC Villangam check online. Learn how to verify property history, check sub-registrar transactions, and locate SRO survey numbers on TNREGINET.',
    'h1_title' => 'EC Villangam: Tamil Nadu Property Encumbrance Certificate Guide',
    'schema_type' => 'Article',
    'faq_data' => '[
        {"question": "How do I perform an EC Villangam search?", "answer": "You can perform an EC Villangam search online by logging into the TNREGINET website, entering the SRO, village name, and the property\'s survey number."},
        {"question": "What is the fee for an online EC Villangam check?", "answer": "Searching and viewing the EC Villangam log on screen is free of charge, but downloading a digitally signed certified copy requires a fee of Rs. 200."},
        {"question": "How long does SRO validation take for an EC Villangam?", "answer": "Online view queries are instant, while certified copies signed by the Sub-Registrar are typically approved and ready in 2 to 3 working days."},
        {"question": "Does a Nil EC Villangam guarantee clear ownership?", "answer": "A Nil EC Villangam shows no registered transactions in the search years, but physical deeds and land revenue registers (Patta) must still be cross-verified."}
    ]',
    'content' => '
<div class="manual-page-container">
    <h1>EC Villangam: Tamil Nadu Property Encumbrance Certificate Guide</h1>
    <p>Retrieving property transaction logs through an online <strong>ec villangam</strong> utility is a major security step for real estate buyers in India. An encumbrance certificate (EC) details all registered transactions on a property, showing whether it is free from liabilities or legal disputes. Digital land registration databases have changed how developers and buyers conduct title due diligence. Today, you can perform an online search and download your certificate from home, saving days of physical SRO search work. Checking the official <a href="/land-ec/">land ec</a> ledger via the <a href="/ec-online/">ec online</a> portal helps confirm that all previous transactions were valued and registered correctly. You can also use the <a href="/tn-ec-view/">TN EC View</a> utility to access SRO logs directly.</p>

    <h2>The Legal Context of Property Registration</h2>
    <p>Under the Indian Registration Act of 1908, any transaction involving the transfer of immovable property must be registered at the local Sub-Registrar Office (SRO). Section 17 lists the documents for which registration is compulsory, including sale deeds, gift deeds, partition deeds, and mortgage deeds. When a deed is registered, it becomes a public record. The SRO indexes these transactions in Book I, which forms the basis for generating an encumbrance certificate. Conducting an <a href="/ec-view/">ec view</a> lookup allows buyers and legal professionals to inspect Book I entries digitally, safeguarding against double-registration fraud and illegal property transfers. Be sure to check the corresponding <a href="/guideline-value-tamilnadu/">guideline value Tamilnadu</a> to verify that the property has not been undervalued during registration.</p>
    <p>The Transfer of Property Act, 1882, under Section 54, defines a sale as a transfer of ownership in exchange for a price paid or promised. For the sale to be complete, a registered instrument is mandatory. Unregistered agreements do not convey any legal title or interest in the property. Therefore, when you perform a <a href="/tn-ec/">TN EC</a> search, you are verifying that the chain of titles consists of validly registered deeds. Any gap in the chain of registered sale deeds indicates a potential title defect that could lead to legal disputes or rejection of home loan applications by financial institutions.</p>
    ...
    <div class="interactive-widgets-section" style="background-color: #f8fafc; padding: 25px; border-radius: 12px; border: 1px solid #e2e8f0; margin-top: 30px;">
        ...
        <div id="app-state-select" class="widget-container" style="margin-bottom: 20px;">
            ...
            <select id="state_selector_ecvillangam" onchange="updateStateInfo_ecvillangam()" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; font-size: 1rem; outline: none; background: #ffffff;">
            ...
            <div id="state_info_box_ecvillangam" style="margin-top: 10px; padding: 12px; border-radius: 6px; background-color: #f1f5f9; display: none;"></div>
        </div>
        ...
        <div id="app-calc-years" class="widget-container" style="margin-bottom: 20px;">
            ...
            <input type="number" id="tenure_years_ecvillangam" min="1" max="50" value="15" oninput="calculateEstimates_ecvillangam()" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; outline: none; box-sizing: border-box;" />
            <div id="calc_results_box_ecvillangam" style="margin-top: 10px; padding: 12px; border-radius: 6px; background-color: #f1f5f9; font-weight: 500;">
        ...
        <div id="app-readiness-deed" class="widget-container">
            ...
            <div style="display: grid; gap: 8px;">
                <label style="display: flex; align-items: center; gap: 8px;"><input type="checkbox" class="chk_ecvillangam" onclick="updateChecklist_ecvillangam()"> Survey / Subdivision Number</label>
                <label style="display: flex; align-items: center; gap: 8px;"><input type="checkbox" class="chk_ecvillangam" onclick="updateChecklist_ecvillangam()"> Sub-Registrar Office (SRO) Name</label>
                <label style="display: flex; align-items: center; gap: 8px;"><input type="checkbox" class="chk_ecvillangam" onclick="updateChecklist_ecvillangam()"> Previous Sale Deed Document Number</label>
            </div>
            <div style="margin-top: 12px; height: 8px; background-color: #e2e8f0; border-radius: 4px; overflow: hidden;">
                <div id="progress_bar_ecvillangam" style="width: 0%; height: 100%; background-color: #10b981; transition: width 0.3s ease;"></div>
            </div>
            <div id="checklist_status_ecvillangam" style="margin-top: 8px; font-size: 0.9rem; font-weight: 500;">Readiness: 0%</div>
        </div>
    </div>

    <script>
    function updateStateInfo_ecvillangam() {
        var selector = document.getElementById("state_selector_ecvillangam");
        var infoBox = document.getElementById("state_info_box_ecvillangam");
    ...
    <script>
    function calculateEstimates_ecvillangam() {
        var years = parseInt(document.getElementById("tenure_years_ecvillangam").value) || 0;
        var cost = 10 + (years * 5);
        document.getElementById("calc_results_box_ecvillangam").innerText = "Estimated search fee: Rs. " + cost.toFixed(2);
    }
    </script>
    <script>
    function updateChecklist_ecvillangam() {
        var chks = document.getElementsByClassName("chk_ecvillangam");
        var checkedCount = 0;
        for (var i = 0; i < chks.length; i++) {
            if (chks[i].checked) checkedCount++;
        }
        var percent = Math.round((checkedCount / chks.length) * 100);
        document.getElementById("progress_bar_ecvillangam").style.width = percent + "%";
        document.getElementById("checklist_status_ecvillangam").innerText = "Readiness: " + percent + "%";
    }
    </script>
</div>
'
