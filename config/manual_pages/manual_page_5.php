<?php
// config/manual_pages/manual_page_5.php
return [
    'slug' => 'ec-download',
    'keyword' => 'ec download',
    'title' => 'EC Download Online: Get Your Encumbrance Certificate Copy',
    'meta_desc' => 'Download your Encumbrance Certificate (EC) online. Learn about state registration portals, SRO document search steps, and verification procedures.',
    'h1_title' => 'EC Download: Online Encumbrance Certificate Copy Retrieval Guide',
    'schema_type' => 'Article',
    'faq_data' => '[
        {"question": "How do I perform an EC download online?", "answer": "You can perform an EC download by logging into the state registration portal, submitting a search for your property survey number, paying the required fees, and downloading the signed PDF."},
        {"question": "Is an online EC download copy legally valid?", "answer": "Yes, if you request and pay for a certified copy, the portal issues a digitally signed PDF copy with a verification barcode that is legally valid for property transfers and loan approvals."},
        {"question": "How long does a digital EC download approval take?", "answer": "An informational search is instant, while a digitally signed certified copy requires validation by the local SRO and is usually ready for download in 2 to 3 days."},
        {"question": "What should I do if my EC download file has missing pages?", "answer": "If pages are missing, it indicates a digital indexing error at the SRO database. You must submit a physical correction request along with copies of your registered deeds."}
    ]',
    'content' => '
<div class="manual-page-container">
    <h1>EC Download: Online Encumbrance Certificate Copy Retrieval Guide</h1>
    <p>Retrieving property transaction logs through an online <strong>ec download</strong> utility is a major security step for real estate buyers in India. An encumbrance certificate (EC) details all registered transactions on a property, showing whether it is free from liabilities or legal disputes. Digital land registration databases have changed how developers and buyers conduct title due diligence. Today, you can perform an online search and download your certificate from home, saving days of physical SRO search work. Conducting a search via <a href="/ec-online/">ec online</a> helps confirm that all previous transactions were valued and registered correctly. You can also use the <a href="/ec-view/">EC view</a> interface to preview your records before downloading.</p>

    <h2>The Legal Context of Property Registration</h2>
    <p>Under the Indian Registration Act of 1908, any transaction involving the transfer of immovable property must be registered at the local Sub-Registrar Office (SRO). Section 17 lists the documents for which registration is compulsory, including sale deeds, gift deeds, partition deeds, and mortgage deeds. When a deed is registered, it becomes a public record. The SRO indexes these transactions in Book I, which forms the basis for generating an encumbrance certificate. Conducting an <a href="/ec-view/">ec view</a> lookup allows buyers and legal professionals to inspect Book I entries digitally, safeguarding against double-registration fraud and illegal property transfers. Ensure you check the property\'s registered value against the official <a href="/check-guideline-value/">guideline value</a> to avoid any stamp duty discrepancy flags.</p>
    ...
    <p>To run a thorough property check, legal professionals also verify Index I and Index II. Index I is a nominal index of executants and claimants, arranged alphabetically. Index II is a descriptive index of properties, categorized by village and survey number. The digital portals retrieve records by querying Index II, which is why a minor error in survey number indexing can omit critical transaction lines. Verification should always include manual validation of the registered document number at the local sub-registrar office if any discrepancies are spotted in the online search results. A complete search via the state portal will list these parameters before you proceed with a certified ec download.</p>
    ...
    <div class="interactive-widgets-section" style="background-color: #f8fafc; padding: 25px; border-radius: 12px; border: 1px solid #e2e8f0; margin-top: 30px;">
        ...
        <div id="app-state-select" class="widget-container" style="margin-bottom: 20px;">
            ...
            <select id="state_selector_ecdownload" onchange="updateStateInfo_ecdownload()" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; font-size: 1rem; outline: none; background: #ffffff;">
            ...
            <div id="state_info_box_ecdownload" style="margin-top: 10px; padding: 12px; border-radius: 6px; background-color: #f1f5f9; display: none;"></div>
        </div>
        ...
        <div id="app-calc-years" class="widget-container" style="margin-bottom: 20px;">
            ...
            <input type="number" id="tenure_years_ecdownload" min="1" max="50" value="15" oninput="calculateEstimates_ecdownload()" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; outline: none; box-sizing: border-box;" />
            <div id="calc_results_box_ecdownload" style="margin-top: 10px; padding: 12px; border-radius: 6px; background-color: #f1f5f9; font-weight: 500;">
        ...
        <div id="app-readiness-deed" class="widget-container">
            ...
            <div style="display: grid; gap: 8px;">
                <label style="display: flex; align-items: center; gap: 8px;"><input type="checkbox" class="chk_ecdownload" onclick="updateChecklist_ecdownload()"> Survey / Subdivision Number</label>
                <label style="display: flex; align-items: center; gap: 8px;"><input type="checkbox" class="chk_ecdownload" onclick="updateChecklist_ecdownload()"> Sub-Registrar Office (SRO) Name</label>
                <label style="display: flex; align-items: center; gap: 8px;"><input type="checkbox" class="chk_ecdownload" onclick="updateChecklist_ecdownload()"> Previous Sale Deed Document Number</label>
            </div>
            <div style="margin-top: 12px; height: 8px; background-color: #e2e8f0; border-radius: 4px; overflow: hidden;">
                <div id="progress_bar_ecdownload" style="width: 0%; height: 100%; background-color: #10b981; transition: width 0.3s ease;"></div>
            </div>
            <div id="checklist_status_ecdownload" style="margin-top: 8px; font-size: 0.9rem; font-weight: 500;">Readiness: 0%</div>
        </div>
    </div>

    <script>
    function updateStateInfo_ecdownload() {
        var selector = document.getElementById("state_selector_ecdownload");
        var infoBox = document.getElementById("state_info_box_ecdownload");
    ...
    <script>
    function calculateEstimates_ecdownload() {
        var years = parseInt(document.getElementById("tenure_years_ecdownload").value) || 0;
        var cost = 10 + (years * 5);
        document.getElementById("calc_results_box_ecdownload").innerText = "Estimated search fee: Rs. " + cost.toFixed(2);
    }
    </script>
    <script>
    function updateChecklist_ecdownload() {
        var chks = document.getElementsByClassName("chk_ecdownload");
        var checkedCount = 0;
        for (var i = 0; i < chks.length; i++) {
            if (chks[i].checked) checkedCount++;
        }
        var percent = Math.round((checkedCount / chks.length) * 100);
        document.getElementById("progress_bar_ecdownload").style.width = percent + "%";
        document.getElementById("checklist_status_ecdownload").innerText = "Readiness: " + percent + "%";
    }
    </script>
</div>
'
