...
return array (
  ...
  'faq_data' => '[{"question":"What services are hosted on registration.telangana.gov.in?","answer":"The portal hosts Encumbrance Search (EC), Market Value Search, Certified Copy (CC) requests, E-Stamping, Property Registration Slot Booking, and Prohibited Property checks."}]',
  'content' => '
<div class="manual-page-container" style="line-height: 1.7; color: var(--text-main);">
    
    <p style="font-size: 1.05rem; margin-bottom: 25px;"><strong>registration.telangana.gov.in</strong> is the primary official portal administered by the Registration and Stamps Department, Government of Telangana.</p>
...
<div class="db-legacy-content" style="margin-top: 30px;"><p class="content-text">
    Property transaction checking in Telangana is managed through the central land registration network. Through the registration telangana gov in portal administered by the stamps department, citizens can inspect deed indices. Using <strong><a href="/ec-online/" title="Read our comprehensive guide on ec online">ec online</a></strong> portals ensures clean property transfers and helps verify SRO transaction logs.
</p>
...
<div id="app-state-select" style="background: linear-gradient(135deg, hsl(210,65%,96%), hsl(210,65%,92%)); border-radius: 12px; padding: 1.5rem; margin: 1.5rem 0; border: 1px solid hsl(210,55%,82%);">
    ...
    <select id="rtg-state-sel" style="padding:0.5rem; border-radius:8px; border:1px solid hsl(210,55%,72%); font-size:1rem; width:100%; max-width:300px; background:#fff;" onchange="showRtgStateInfo()">
    ...
    <div id="rtg-state-result" style="margin-top:1rem; font-size:0.95rem; color:hsl(210,55%,22%); line-height:1.5;"></div>
</div>
<script>
function showRtgStateInfo() {
    var s = document.getElementById("rtg-state-sel").value;
    var r = document.getElementById("rtg-state-result");
...
<div id="app-calc-years" style="background: linear-gradient(135deg, hsl(160,65%,96%), hsl(160,65%,92%)); border-radius: 12px; padding: 1.5rem; margin: 1.5rem 0; border: 1px solid hsl(160,55%,82%);">
    ...
    <input type="number" id="rtg-years-inp" min="1" max="40" value="15" style="width:90px; padding:0.5rem; border-radius:8px; border:1px solid hsl(160,55%,72%); font-size:1rem; text-align:center;" oninput="calcRtgYears()">
    <span style="margin-left:0.5rem; color:hsl(160,45%,28%);">years</span>
    <div id="rtg-years-result" style="margin-top:0.8rem; font-weight:600; color:hsl(160,55%,22%); font-size:0.95rem;"></div>
</div>
<script>
function calcRtgYears() {
    var y = parseInt(document.getElementById("rtg-years-inp").value) || 0;
    var r = document.getElementById("rtg-years-result");
...
<div id="app-readiness-deed" style="background: linear-gradient(135deg, hsl(35,80%,97%), hsl(35,70%,93%)); border-radius: 12px; padding: 1.5rem; margin: 1.5rem 0; border: 1px solid hsl(35,60%,85%);">
    ...
    <div style="display:flex; flex-direction:column; gap:0.6rem;">
        <label style="display:flex; align-items:center; gap:0.5rem; font-size:0.9rem; color:hsl(35,55%,25%); cursor:pointer;">
            <input type="checkbox" id="rtg-chk1" onchange="runRtgCheck()" style="width:16px; height:16px;"> District name, SRO location, and village name
        </label>
        <label style="display:flex; align-items:center; gap:0.5rem; font-size:0.9rem; color:hsl(35,55%,25%); cursor:pointer;">
            <input type="checkbox" id="rtg-chk2" onchange="runRtgCheck()" style="width:16px; height:16px;"> Valid Survey number and subdivision code
        </label>
        <label style="display:flex; align-items:center; gap:0.5rem; font-size:0.9rem; color:hsl(35,55%,25%); cursor:pointer;">
            <input type="checkbox" id="rtg-chk3" onchange="runRtgCheck()" style="width:16px; height:16px;"> Registered document number and year
        </label>
    </div>
    <div id="rtg-chk-result" style="margin-top:0.8rem; padding:0.7rem 1rem; border-radius:8px; background:#fff8f1; border:1px solid hsl(30,70%,80%); font-size:0.9rem; color:hsl(30,55%,28%); font-weight:600;">
        Confirm search inputs are verified.
    </div>
</div>
<script>
function runRtgCheck() {
    var c1 = document.getElementById("rtg-chk1").checked;
    var c2 = document.getElementById("rtg-chk2").checked;
    var c3 = document.getElementById("rtg-chk3").checked;
    var r = document.getElementById("rtg-chk-result");
